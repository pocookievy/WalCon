...
// auth-hotfix-v11-src.js
var TEMPLATE_VERSION = 11;
var app = initializeApp(firebaseConfig, "walcon-auth-hotfix-v11");
...
var DEFAULT_CATEGORIES = [
  ...
  { id: "debt-personal-loans", group: "debt", name: "Personal Loans", icon: "\u{1F4C9}", lineItems: ["Payment"] }
];
var DEFAULT_SOURCES = [
...
function busy(value) {
  for (const selector of ["#loginBtn", "#signupBtn", "#forgotPasswordBtn"]) {
    const el = $2(selector);
...
async function login() {
  ...
  const password = $2("#authPassword")?.value || "";
  if (!email || !password) {
...
function installForgotButton() {
  if ($2("#forgotPasswordBtn")) return;
  const msg = $2("#authMessage");
  if (!msg) return;
...
async function handleDeleteCapture(event) {
  ...
  if (catBtn) {
    ...
    try {
      await markDeleted("category", catBtn.dataset.deleteCategory);
      await deleteDoc(doc(db, "users", auth.currentUser.uid, "categories", catBtn.dataset.deleteCategory));
    } catch (error) {
      console.error("Category delete hotfix failed", error);
    }
    return;
  }
  ...
  if (sourceBtn) {
    ...
    try {
      await markDeleted("source", sourceBtn.dataset.deleteSource);
      await deleteDoc(doc(db, "users", auth.currentUser.uid, "balanceSources", sourceBtn.dataset.deleteSource));
    } catch (error) {
      console.error("Balance source delete hotfix failed", error);
    }
  }
}
async function init() {
  ...
  installForgotButton();
  const loginBtn = $2("#loginBtn");
  const signupBtn = $2("#signupBtn");
  if (loginBtn) {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (!user) return;
    try {
      await seedTemplate(user);
    } catch (error) {
      console.error("WalCon template seed failed", error);
    }
    const authActive = document.querySelector('[data-view="auth"].active');
    const key = sessionStorage.getItem("walcon-auth-sync-v11");
    if (authActive && key !== user.uid) {
      sessionStorage.setItem("walcon-auth-sync-v11", user.uid);
      location.reload();
    }
  });
...
init().catch((error) => {
  console.error("WalCon auth hotfix failed", error);
  authMessage(`Authentication setup failed: ${error?.message || error}`);
});
